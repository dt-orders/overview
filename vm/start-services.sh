...
sudo $APP_SCRIPTS_PATH/stop-services.sh $SCRIPT_HOME | tee -a $LOGFILE

echo "***** Starting Services ***" | tee -a $LOGFILE
sudo docker-compose -f "$APP_SCRIPTS_PATH/docker-compose-services.yaml" up -d | tee -a $LOGFILE
